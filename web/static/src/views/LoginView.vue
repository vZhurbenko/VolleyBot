<template>
  ...
  >
    <div class="bg-white rounded shadow p-6 lg:p-10 w-full max-w-md">
      <img :src="logo" alt="Team R Logo" class="w-20 mx-auto mb-4" />
...
const hasTrainingRedirect = computed(() => {
  ...
  return (
    redirect &&
    (redirect.match(/\/t\/([a-f0-9-]+)/i) || redirect.match(/\/guest\/training\/([a-f0-9-]+)/i))
  );
});
...
const handleRedirectOrAdmin = () => {
  ...
  if (redirect && trainingRedirectMatch) {
    ...
    if (isGuest) {
      ...
      console.log(
        "[LoginView] Пользователь редиректится на календарь:",
        `/dashboard/calendar?training=${uuid}`,
      );
  ...
  try {
    ...
    if (response.ok && result.success) {
      ...
      if (redirect && trainingRedirectMatch) {
        ...
        if (isGuest) {
          console.log(
            "[LoginView] Гость редиректится на страницу гостя:",
            `/guest/training/${uuid}`,
          );
          window.location.href = `/guest/training/${uuid}`;
        } else {
          ...
          console.log(
            "[LoginView] Пользователь/админ редиректится на календарь:",
            `/dashboard/calendar?training=${uuid}`,
          );
          window.location.href = `/dashboard/calendar?training=${uuid}`;
        }
    ...
    if (response.ok) {
      ...
      if (redirect) {
        ...
        if (trainingRedirectMatch) {
          ...
          if (isGuest) {
            console.log(
              "[LoginView] Гость перенаправляется на страницу гостя:",
              `/guest/training/${uuid}`,
            );
            window.location.href = `/guest/training/${uuid}`;
          } else {
            ...
            console.log(
              "[LoginView] Пользователь/админ перенаправляется на календарь:",
              `/dashboard/calendar?training=${uuid}`,
            );
            window.location.href = `/dashboard/calendar?training=${uuid}`;
          }
